...
export default function AdminHeroPage() {
  ...
  const addByUrl = () => {
    ...
    if (parseYouTubeVideoId(u)) {
      const id = crypto.randomUUID();
      setSlides((prev) => [
        ...prev,
        { id, ...HERO_LOCAL_DEFAULT_VIDEO },
      ]);
      setUrlInput("");
      setStatus(
        "YouTube URL은 히어로에 사용하지 않습니다. public/의 로컬 영상(hero_1.mp4, mobile_hero_1.mp4)으로 추가했습니다. 저장을 눌러 반영하세요.",
      );
      return;
    }
    ...
    if (!isAllowedHeroExtension(ext)) {
      setStatus(
        `확장자가 허용 목록에 있어야 합니다. (${HERO_ALLOWED_EXTENSIONS.join(", ")})`,
      );
      return;
    }
    const id = crypto.randomUUID();
        ...
        <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-zinc-400">
              mp4, gif, webp, png, jpeg, jpg · (구)YouTube URL은 자동으로 로컬
              mp4(hero)로 바뀝니다 · 라이브 사이트는{" "}
        ...
        <div className="rounded-2xl border border-white/10 bg-zinc-900/60 p-6 shadow-xl backdrop-blur">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-zinc-500">
                이미지·동영상 URL 또는(구)YouTube 링크 — YouTube는 로컬 히어로
                mp4로 저장됩니다
              </p>
              <div className="mt-2 flex gap-2">
                <input
                  ...
                  placeholder="https://…/banner.mp4 또는 /hero_1.mp4"
              ...
              slides.map((s, i) => {
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {(s.kind === "video" || s.kind === "youtube") && s.mobileUrl ? (
